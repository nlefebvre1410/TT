...
namespace Cz\ManagerBundle\Routing;

use Cz\ManagerBundle\Helper\Utils\DomainConfigurationInterface;
use Acme\ContentBundle\Entity\Content;
use Acme\ContentBundle\Entity\ContentTranslation;
use Acme\ContentBundle\Repository\ContentRepository;
...
class SlugRouter implements RouterInterface
{
    ...
    const CLASS_NODE =  'AcmeContentBundle:Content';
    ...
    public function __construct($container)
    {
        ...
        $this->domainConfiguration = $container->get('cz_manager.domain_configuration');
    }

    /**
     * Associez urls données via le contexte des routes que nous avons définies .
     * Cette fonctionnalité re-utilise le Symfony mode de routage et sa valeur par défaut
     * Composants
     *
    ...
    protected function getPreviewRouteParameters()
    {
        ...
        $previewDefaults     = array(
            '_controller' => 'CzManagerBundle:Slug:slug',
            'preview'     => true,
            'url'         => '',
    ...
    protected function getNodeTranslationRepository()
    {
        ...
        $nodeTranslationRepo = $em->getRepository( self::CLASS_NODE );
